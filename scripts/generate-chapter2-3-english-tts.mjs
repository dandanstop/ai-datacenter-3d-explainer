...
function englishSection(markdown) {
  const lines = markdown.split(/\r?\n/);
  const headingIndex = lines.findIndex((line) => line.trim() === "## English");
  if (headingIndex === -1) throw new Error("Missing English section.");
...
for (const chapter of chapters) {
  ...
  const section = englishSection(markdown);
