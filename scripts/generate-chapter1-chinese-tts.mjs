...
function extractChineseTranscript(markdown, heading) {
  ...
  const chineseMatch = section.match(/### zh\s+([\s\S]*?)(?=\n### |\n## |$)/);
  if (!chineseMatch) {
    throw new Error(`Missing Chinese transcript for: ${heading}`);
  }
  return chineseMatch[1].trim().replace(/\s+/g, " ");
}
...
const jobs = segments.map((segment) => ({
  ...segment,
  text: extractChineseTranscript(markdown, segment.heading)
}));
