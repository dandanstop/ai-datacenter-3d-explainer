...
function extractKoreanTranscript(markdown, heading) {
  ...
  const koreanMatch = section.match(/### ko\s+([\s\S]*?)(?=\n### |\n## |$)/);
  if (!koreanMatch) {
    throw new Error(`Missing Korean transcript for: ${heading}`);
  }
  return koreanMatch[1].trim().replace(/\s+/g, " ");
}
...
const jobs = segments.map((segment) => ({
  ...segment,
  text: extractKoreanTranscript(markdown, segment.heading)
}));
...
for (let index = 0; index < jobs.length; index += 1) {
  ...
  const outPath = path.join(outputDir, `ko-${segment.id}.mp3`);
